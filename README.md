...
## `DDD` (Domain-Driven Design)
  ...
  - Данный подход предполагает владение знаниями о конкретной предметной области бизнеса и подразумевает создание модели ПО, которая её точно отражает.
    - Предметная облась - это то, как компания ведёт своё собственное дело: её организация, процессы, методы люди и даже язык.
...
Из чего состоит данный подход
  - Аналитическая часть
    - Единый язык - это словарь, которого последовательно придерживаются все участники проекта. Это также _шаблон_
, влияющий на выбор имён и структуры классов. Он помогает лучше и быстрее понять требования и успростить связь мужде сторонами воизбежание недопониманий.
    ...
    - Создание карты контекстов - создаёт высокоуровневое представление области с точки зрения аритектора ПО, выявляет подобласти и отношения между ними.
...
## Архитектуры приложений:
  - **Многоуровневая архитектура (Multilayer Architecture)**
    - Разделение приложения на уровень прежставения данных, бизнес-логики и самих данных.
  - **Многоярусная архитектура (Multitier Architecture)**
     - Архитектура, похожая на многоуровневую за исплючением того, что вместо уровней в ней используются ярусы.
  ...
  - **Источник событий (Event Sourcing)**
     - Многослойная архитектура, которая практически всегда основывается на принципе CQRS, а не на данных. События интерпретируются как данные первого класса, а вся запрашиваемая информация выврдится из хранимых событий.
...
Различие между уровнями (layers) и ярусами (tier):
  - _Уровень_ - это логический контейнет для различных частей кода
  - _Ярус_ - это физический контекнер для кода, и относится к своему собственному пространству процесса или машины.
...
Многоуровневая и Многоярусная архитектуры отличаются друг от друга тем, что в первой блоки отделяются друг от друга логичеки, а во второй - физически.
...
Зачастую при написании многоуровневого приложеия используется 2 яруса: Представление данных && Бизнес логика и отдельно Данные.

### Улучшенная (современная) многоуровневая архитектура
После появления Модели Предметной области и DDD, классическая многоуровневая архитектура притерпела некоторые изменения:
...
Также бизнес-логика вышла за пределы своего слоя и стала реализовываться как в на уровне представления, так и на уровне данных.
Вышеупомянутые изменения говорят о том, что для классической многоуровневой арзитектуры был применён принцип разделения функций (SoC - Separation of Concerns).
...
Немного о самих уровнях и об их обязанностях:
 - Уровень представления
   ...
   - Уровень представления аппелирует моделями представления, и зачастую эти модели совпадают с моделями входных данных, с которыми уже работает Прикладной уровень.
 - Прикладной уровень 
   - Дополнительный слой, оторый передаёт информацию представлению и синхронизирует выполнение всех других бизнес-операций.
   ...
   - Этот уровень содержит ссылки на уровень предметной области и инфраструктурный уровень. Кроме того, этот уровени ничего не знает о бизнес-правилах и не хранит никакой бизнес-информации.
 ...
 - Инфраструктурный уровень
   - Связаи с использованием конкретных технологий хранения данных (ORM), механизмов обеспечения безопасности, регистрации, трассировки, кеширования , контейнеров IoC и т.д.
